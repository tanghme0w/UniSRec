r"""
based on recbole.trainer.trainer
################################
"""
...
class AbstractTrainer(object):
    ...
    def fit(self, train_data):
        r"""Train the model based on the valid data."""
...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        Args:
            train_data (DataLoader): The valid data.
        ...
        """
        self.model.train()
        loss_func = self.model.calculate_missrec_loss
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):
        ...
        if isinstance(losses, tuple):
            ...
            des = "%." + str(des) + "f"
            train_loss_output += set_color("valid loss", "blue") + ": " + des % losses
    ...
    def fit(
        ...
        r"""Train the model based on the valid data and the valid data.
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        scores = self.model.mmap_full_sort_predict(interaction.to(self.device), self.eval_dataloader.plm_embedding)

        scores = scores.view(-1, self.tot_item_num)
        scores[:, 0] = -np.inf
        if history_index is not None:
            scores[history_index] = -np.inf
        positive_i = self.eval_dataloader.plm_embedding.idx_convert(positive_i)
